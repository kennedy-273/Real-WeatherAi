...
import { searchCities, fetchWeather, conditionLabel, moodFromCondition, type GeoResult, type WeatherResponse } from "@/lib/weather";
import { Search, MapPin, Wind, Droplets, Sun, Gauge, Loader2, Sparkles, Thermometer, Star } from "lucide-react";

const DEFAULT_CITY: GeoResult = {
  id: 1, name: "Nairobi", country: "Kenya", country_code: "KE", admin1: "Nairobi County",
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
              ...
              {(["metric", "imperial"] as const).map((u) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 transition-colors ${
                    units === u ? "bg-foreground text-background" : "text-foreground/70 hover:text-foreground"
                  }`}
                >
                  {u === "metric" ? "°C" : "°F"}
                </button>
          ...
          <div ref={boxRef} className="relative mx-auto mt-8 max-w-xl">
            <div className="flex items-center gap-3 rounded-2xl glass-strong px-4 py-3">
              ...
              <input
                value={query}
                onChange={(e) => { setQuery(e.target.value); setShowResults(true); setActiveIndex(null); }}
                ...
                <div className="grid grid-cols-2 gap-2 rounded-2xl overflow-hidden bg-transparent">
                  <div className="max-h-64 overflow-y-auto bg-transparent p-2">
                    {results.length > 0 ? (
                          ...
                          <li key={r.id}>
                            ...
                            >
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="font-medium">{r.name} <span className="text-foreground/60">{r.admin1 ? `· ${r.admin1}` : ''}</span></div>
                                  <div className="mt-0.5 text-xs text-foreground/60">{r.country} · {r.timezone}</div>
                                </div>
                              </div>
                              <div className="flex items-center gap-2">
                                ...
                                >
                                  <Star className={`h-4 w-4 ${isFavorite(r) ? 'text-accent' : ''}`} />
                                </button>
                                <span className="text-xs text-foreground/60">{r.latitude.toFixed(2)}, {r.longitude.toFixed(2)}</span>
                              </div>
                            </button>
                          </li>
                        ))}
                      ...
                      <div className="p-4 text-sm text-foreground/60">{searching ? 'Searching…' : (query.trim() ? 'No results' : 'Type to search cities')}</div>
                    )}
                  </div>
                  ...
                  <div className="border-l border-white/6 p-3">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <button className="text-xs text-foreground/50 hover:text-foreground" onClick={() => { setRecent([]); }}>Clear</button>
                    </div>
                    <div className="flex flex-col gap-2">
                      {recent.length ? recent.map((r) => (
                        <button key={r.id} onClick={() => selectResult(r)} className="text-left text-sm hover:bg-white/6 rounded-md px-2 py-1">{r.name} <span className="text-foreground/60">· {r.country}</span></button>
                      )) : (
                        <div className="text-sm text-foreground/60">No recent searches</div>
                      )}

                      <div className="mt-3 border-t border-white/6 pt-3 text-xs text-foreground/70">Favorites</div>
                      <div className="mt-2 flex flex-wrap gap-2">
                        {favorites.length ? favorites.map((f) => (
                          <button key={f.id} onClick={() => selectResult(f)} className="rounded-full bg-white/6 px-3 py-1 text-sm">{f.name}</button>
                        )) : (
                          <div className="text-sm text-foreground/60">No favorites yet — click the <Star className="inline h-4 w-4" /> icon to add one.</div>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <>
                ...
                <section className="grid gap-6 lg:grid-cols-3">
                  <div className="rounded-3xl glass-strong p-8 lg:col-span-2">
                    ...
                    <div className="mt-6 flex flex-wrap items-end gap-8">
                      ...
                      <div className="ml-auto text-right text-sm text-foreground/70">
                        <div>{new Date(data.current.time).toLocaleString(undefined, { weekday: "long", hour: "2-digit", minute: "2-digit" })}</div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <Tile icon={<Thermometer className="h-4 w-4" />} label="Feels like" value={`${Math.round(data.hourly[0]?.feels_like ?? data.current.temperature)}${tempUnit}`} />
                    <Tile icon={<Droplets className="h-4 w-4" />} label="Humidity" value={`${data.hourly[0]?.humidity ?? "—"}%`} />
                    <Tile icon={<Wind className="h-4 w-4" />} label="Wind" value={`${Math.round(data.current.wind_speed)} ${windUnit}`} />
                    <Tile icon={<Sun className="h-4 w-4" />} label="UV index" value={`${Math.round(data.hourly[0]?.uv_index ?? 0)}`} />
                    <Tile icon={<Gauge className="h-4 w-4" />} label="Gust" value={`${Math.round(data.hourly[0]?.wind_gust ?? 0)} ${windUnit}`} />
                    <Tile icon={<Droplets className="h-4 w-4" />} label="Rain chance" value={`${data.daily[0]?.precipitation_probability ?? 0}%`} />
                  </div>
                </section>

                {/* Hourly */}
                <section className="mt-8 rounded-3xl glass p-6">
                  <h2 className="mb-4 text-sm font-medium uppercase tracking-wider text-foreground/70">Next 24 hours</h2>
                  <div className="flex gap-2 overflow-x-auto pb-2">
                    {hourlySlice(data).map((h) => (
                      <div key={h.time} className="flex min-w-[78px] flex-col items-center rounded-2xl bg-white/5 px-3 py-3">
                        <div className="text-xs text-foreground/70">
                          {new Date(h.time).toLocaleTimeString(undefined, { hour: "numeric" })}
                        </div>
                        <img src={h.icon} alt="" className="my-2 h-10 w-10" />
                        <div className="text-base font-medium">{Math.round(h.temperature)}{tempUnit}</div>
                        <div className="mt-1 text-[10px] text-foreground/60">{h.precipitation_probability}%</div>
                      </div>
                    ))}
                  </div>
                </section>
                ...
                <section className="mt-6 rounded-3xl glass p-6">
                  <h2 className="mb-4 text-sm font-medium uppercase tracking-wider text-foreground/70">7-day forecast</h2>
                  <ul className="divide-y divide-white/10">
                    {data.daily.slice(0, 7).map((d, i) => {
                      const tMin = Math.round(d.temp_min);
                      const tMax = Math.round(d.temp_max);
                      return (
                        <li key={d.date} className="grid grid-cols-[110px_56px_1fr_110px] items-center gap-4 py-3 text-sm">
                          <span className="text-foreground/80">
                            {i === 0 ? "Today" : new Date(d.date).toLocaleDateString(undefined, { weekday: "long" })}
                          </span>
                          <img src={d.icon} alt="" className="h-10 w-10" />
                          <span className="text-foreground/70">{conditionLabel(d.condition_code)}</span>
                ...
                <section className="mt-6 grid grid-cols-2 gap-4">
                  <div className="rounded-2xl glass p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="mt-1 font-medium">{data.daily[0]?.sunrise ? new Date(data.daily[0].sunrise).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'} · {data.daily[0]?.sunset ? new Date(data.daily[0].sunset).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}</div>
                    </div>
                  </div>
                  <div className="rounded-2xl glass p-4 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="mt-1 font-medium">{Math.round(data.daily.slice(0,7).reduce((s, d) => s + (d.precipitation_sum ?? 0), 0))} mm</div>
                    </div>
                  </div>
                </section>

                <footer className="mt-10 pb-6 text-center text-xs text-foreground/60">
                  Data: <a className="underline hover:text-foreground" href="https://weather-ai.co/docs" target="_blank" rel="noreferrer">weather-ai.co</a> · Geocoding: open-meteo
                </footer>
              </>
            )}
          </main>
